...
//        }
        ...
        for (int i = 0; i < 4; i++) {
            if(Utils.getRelativePoint(enemyPosition, 1, Utils.getDirectionsAsArray()[i]).x >= 0 && Utils.getRelativePoint(enemyPosition, 1, Utils.getDirectionsAsArray()[i]).y >= 0
                ...
                int heuristicValue = AIHeuristics.getAIHeuristics(arenaTiles, this.gameCharacterSet, enemies)[positionToCheck.x][positionToCheck.y];
                ...
                if (heuristicValue > nextDirection.value) {
                    ...
                    nextDirection.value = AIHeuristics.getAIHeuristics(arenaTiles, this.gameCharacterSet, enemies)[positionToCheck.x][positionToCheck.y];
                }
            } else {
                int x = random.nextInt(4);
                switch(x) {
